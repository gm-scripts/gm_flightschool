...
const configLoaded = (): void => {
  exams.push({
    label: lang["menu"].exams.plane,
    id: 0,
    complete: false,
    price: conf["prices"].allFree
      ? lang["free"]
      : lang["currency"].replace("_price_", conf["prices"].practice),
  });
  isConfigSynced = true;
      ...
      if (
        ...
        if (IsControlJustPressed(0, keys.openMenu)) {
          serverCallback(`gm_${script}:getLicenses_${conf["framework"]}`, {}, licenses => {
            ...
            for (let i = 0; i < licenses.length; i++) {
              if (
                licenses[i].type === "drive_plane" ||
                licenses[i].type === "dmv_plane"
              ) {
                let license = "none";
                switch (licenses[i].type) {
                  case "drive_plane":
                    license = lang["menu"].exams.plane;
                    break;
                  case "dmv_plane":
                    found = true;
                    break;
